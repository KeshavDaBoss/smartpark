...
ACTIVE_PI_SLOTS = set(PI_PINOUT.keys())
...
def setup_gpio():
    ...
    for slot_id, pins in PI_PINOUT.items():
        ...
        try:
            ...
            ACTIVE_PI_SLOTS.discard(slot_id)
            print(f"GPIO Setup Error for {slot_id}: {e}")
            print(f"Disabling hardware polling for {slot_id} and continuing startup.")
...
def update_pi_sensors():
    ...
    for slot_id, pins in PI_PINOUT.items():
        if slot_id not in ACTIVE_PI_SLOTS:
            continue
        ...
        try:
            ...
            ACTIVE_PI_SLOTS.discard(slot_id)
            print(f"GPIO Read Error for {slot_id}: {e}")
            print(f"Disabling hardware polling for {slot_id}.")
            continue
...
def set_led(slot_id: str, state: bool):
    ...
    if pins and pins['led'] is not None:
        try:
            ...
            ACTIVE_PI_SLOTS.discard(slot_id)
            print(f"GPIO LED Error for {slot_id}: {e}")
